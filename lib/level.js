const {Auto} = require('./obstacle.js');
...
class Level {
  ...
  buildRoadLevel() {
    const fastCarImg = new Sprite(57, 1123, 50, 47);
    const semiImg = new Sprite(223, 1130, 90, 33);
    const slowCarImg = new Sprite(350, 1130, 50, 33);
    const tractorImg = new Sprite(138, 1127, 45, 40);
    let autos = []

    switch (this.currentLevel) {
    case 1:
      autos = this.buildRoadRow(Auto, 614.5, 1, 2, 8, 2, 4, 'black', tractorImg);
      autos = autos.concat(this.buildRoadRow(Auto, 564.5, 1, -1, 4, 4, 4, 'black', slowCarImg))
      autos = autos.concat(this.buildRoadRow(Auto, 514.5, 1, 3, 3, 4, 1, 'black', fastCarImg));
      autos = autos.concat(this.buildRoadRow(Auto, 464.5, 2, -3, 3, 5, 3, 'black', semiImg));
      break;
    case 2:
      autos = this.buildRoadRow(Auto, 614.5, 1, 2, 4, 4, 4, 'black', tractorImg);
      autos = autos.concat(this.buildRoadRow(Auto, 564.5, 1, -1, 4, 4, 4, 'black', slowCarImg))
      autos = autos.concat(this.buildRoadRow(Auto, 514.5, 1, 3, 3, 4, 1, 'black', fastCarImg));
      autos = autos.concat(this.buildRoadRow(Auto, 464.5, 2, -3, 3, 5, 3, 'black', semiImg));
      break;
    case 3:
      autos = this.buildRoadRow(Auto, 614.5, 1, 3, 4, 4, 4, 'black', tractorImg);
      autos = autos.concat(this.buildRoadRow(Auto, 564.5, 1, -1, 4, 4, 4, 'black', slowCarImg))
      autos = autos.concat(this.buildRoadRow(Auto, 514.5, 1, 15, 3, 2, 1, 'black', fastCarImg));
      autos = autos.concat(this.buildRoadRow(Auto, 464.5, 2, -3, 3, 5, 3, 'black', semiImg));
      break;
    }
    return autos;
