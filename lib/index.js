...
function collisionDetection() {
  ...
  //  newToad.height + newToad.yCoord > car.yCoordinate) {
    ...
    if ((newToad.yCoord + 2 == car.yCoordinate) &&
    ((newToad.xCoord < car.xCoordinate + car.width && newToad.xCoord > car.xCoordinate) ||
    (newToad.xCoord + newToad.width > car.xCoordinate && newToad.xCoord + newToad.width < car.xCoordinate + car.width))) {
      newToad.status = 'dead';
    // console.log('toad is dead');
    }
  });
  let toadLog = logs.filter(function(log) {
    return (newToad.yCoord + 2 == log.yCoordinate) &&
      newToad.xCoord > log.xCoordinate - newToad.margin &&
      newToad.xCoord + newToad.width < log.xCoordinate + log.width + newToad.margin
  }, newToad);
  let turtleLog = turtles.filter(function(turtle) {
    return !turtle.submerged &&
    newToad.yCoord + 2 == turtle.yCoordinate &&
    newToad.xCoord > turtle.xCoordinate - newToad.margin &&
    newToad.xCoord + newToad.width < turtle.xCoordinate + turtle.width + newToad.margin
  }, newToad)

  if (toadLog.length) {
    newToad.velocity = toadLog[0].velocity;
  }
  if (turtleLog.length) {
    newToad.velocity = turtleLog[0].velocity;
  }
  if (!toadLog.length && !turtleLog.length && newToad.yCoord < 325) {
    newToad.status = 'dead';
  }
}
