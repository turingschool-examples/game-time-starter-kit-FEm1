...
class Game {
  ...
  buildRoadRow(vehicleType, yCoordinate, carLength, speed, gap, numCars, offset) {
  ...
  buildHomes() {
    let homeyCoordinate = 25;
    let startxCoordinate = this.squareWidth - 2.5;
    let gap = this.squareWidth * 2;
    let numberOfHomes = 5;
    let buildHomeArray = [];

    for (let i = 0; i < numberOfHomes; i++) {
      buildHomeArray.push(new Home(startxCoordinate + (this.squareWidth + gap) * i, homeyCoordinate, this.squareWidth, this.squareHeight));
    }
