...
class Game {
  ...
  //     this.drawBackground();
  //     this.drawGamePieces();
  //     this.score.drawScore(context);
  //     this.collisionDetection();
  //     this.toad.drawToad(context);
  //     this.checkForWin();
  ...
  checkForWin() {
    let emptyHomes = this.homes.filter(function(home) {
      return !home.hasToad;
    })
  ...
  buildHomes() {
    ...
    for (let i = 0; i < numberOfHomes; i++) {
      buildHomeArray.push(new Home(startXcoord + (this.squareWidth + gap) * i,
      homeYcoord, this.squareWidth, this.squareHeight, this.timer, this.score));
    }
    console.log(buildHomeArray);
